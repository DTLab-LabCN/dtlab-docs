...
function iterParents(elm: HTMLElement, cb: (elm: HTMLElement) => boolean|void) {
  while (elm) {
...
// Scroll-spy
let selected: HTMLElement, lastAnchor: HTMLElement;
function scrollSpy() {
...
const positionSpy = (() => {
  const originalTop: number = sidebar.offsetTop;
  const originalMargin: string = getComputedStyle(sidebar).marginTop;
